...
def edit(request, id):
    ...
    return render_to_response('alertconfig.html',
                              {'pagedata': pagedata},
                              context_instance=RequestContext(request))
...
def pay_it_status(request):
    return render_to_response('pay_it_status.html',
...
def task_report(request):
    ...
    report_list=[]    
    if request.method == 'POST':
        list_header=['UserName','ProjectName','Tasks','StartTime','TimeSpent']
        fromDate = datetime.strptime(request.POST.get('fromDate', ''),\
                                                        '%m-%d-%Y')
        toDate = datetime.strptime(request.POST.get('toDate', ''),\
                                    '%m-%d-%Y') + relativedelta(days=+1)

        timesheet_list = TaskTracking.objects.filter(start_time__range=\
                    [datetime.strftime(fromDate, '%Y-%m-%d'),\
                    datetime.strftime(toDate, '%Y-%m-%d')])
        task_list = dict(Task.objects.filter(id__in =\
            [task[0] for task in timesheet_list.values_list('task')]\
            ).values_list('id','name'))
        project_list=dict(Project.objects.filter(id__in = \
            [project[0] for project in timesheet_list.values_list('program')]\
            ).values_list('id','name'))        
        for i, task in enumerate(timesheet_list):
            ...
            report_list.append([task.user.username,project_name, task_name, \
                datetime.strftime(task.start_time, '%Y-%m-%d %H:%M:%S'), 
                                 task.time_spent])
