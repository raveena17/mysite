...
def SaveAnnouncement(request):
    ...
    return render_to_response('CreateAnnouncement.html',
...
def ViewAnnouncement(request):
    ...
    return render_to_response('AnnouncementView.html',
...
def ShowApprovedBy(request):
    ...
    return render_to_response('CreateAnnouncement.html',{'approved_by' : approved_by, 'requested_by' : requested_by, 'requested_by_id' : requested_by_id},context_instance = RequestContext(request))
            ...
            for each in UserProfile.objects.filter(Q(code__startswith="E") | Q(user__username='pvj')):
                if(each.user.is_active == True):
                    if(each.user.email.endswith(ommission) != True):
                        email_to_list.append(each.user.email)                        
                        #Email().send_email(email_subject1, email_message1, [each.user.email], CONTENT_TYPE)          
            alert_generate(request, 'Announcement', 'alertdataconfig15',announcement.pk, email_to_list)
            msg = 'APPROVED'
        except Exception:
            errMessage = 'Email Sending failed \n %s' % (Exception)
