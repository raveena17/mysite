...
def login(request, template_name = 'login.html',
    ...
    redirect_to = settings.USER_REDIRECT_URL
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.user and request.user.is_staff:
                redirect_to = settings.ADMIN_REDIRECT_URL
...
@login_required
@permission_required('users.add_userprofile')
def manage_user(request, pid = None, redirect_to = user_list):
    """
        Add/Edit user.
    """
    ...
    return render_to_response('user.html', {'profile':profile,
